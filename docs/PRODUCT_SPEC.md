...
| ---- | ---------------------------------------------------------------------------------- | ------ | ------ |
| U-01 | アカウント登録・ログイン                                                           | 高     | ○      |
| U-02 | 自分のパーティ登録(6体、技・持ち物・特性・性格・能力ポイント・実数値・テラス/メガ) | 高     | ○(1つ) |
| U-03 | 複数パーティ保存・切替                                                             | 中     | −      |
| U-04 | パーティ編集・削除                                                                 | 高     | ○      |
| U-05 | 対戦記録の保存・閲覧                                                               | 中     | −      |
| U-06 | 構築のお気に入り登録                                                               | 低     | −      |
...
| evs          | jsonb nullable  | 従来Partyとの互換データ。新規Champions Partyではnull                   |
| stat_points  | jsonb nullable  | Pokémon Championsの能力ポイント(各0〜32・合計66以下)                   |
| ivs          | jsonb nullable  | 従来Partyとの互換データ。nullは未確認であり、31へ補完しない            |
...
`battle_level`を組み合わせる。新規Partyはゲーム画面で確認した能力ポイントと6能力の実数値を
直接入力し、`stat_points`と`actual_stats`へそれぞれ保存する。能力ポイントから実数値を算出せず、
実数値から能力ポイント・EV・IVを逆算しない。新規Champions Partyの`evs`と`ivs`はnullとする。
既存PartyのEV・IVは互換データとして保持するが`stat_points`へ自動変換せず、DB直接操作等により
`actual_stats=null`が残る場合はcounterplan計算不能な不整合として扱う。
...
- ポケモン選択→技4つ→持ち物→特性→性格→能力ポイント→実数値→テラス/メガ
- 能力ポイントは各0〜32・合計66以下を数値入力し、合計を常時表示する
- 実数値はゲーム画面のHP・攻撃・防御・特攻・特防・素早さを直接入力する
- 通常フローでは従来EV・IVを入力せず、能力ポイントから実数値を自動計算しない
